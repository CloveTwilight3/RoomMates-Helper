...
/**
 * Load pending confessions from file on startup
 */
function loadPendingConfessions() {
    try {
        if (require('fs').existsSync(CONFESSIONS_FILE)) {
            const data = require('fs').readFileSync(CONFESSIONS_FILE, 'utf8');
            const confessionsArray = JSON.parse(data);
            
            // Convert array back to Map
            confessionsArray.forEach(([key, value]: [string, ConfessionData]) => {
                pendingConfessions.set(key, value);
            });
            
            console.log(`✅ Loaded ${pendingConfessions.size} pending confessions from storage`);
        }
    } catch (error) {
        console.error('❌ Error loading pending confessions:', error);
    }
}

/**
 * Save pending confessions to file
 */
function savePendingConfessions() {
    try {
        const confessionsArray = Array.from(pendingConfessions.entries());
        require('fs').writeFileSync(CONFESSIONS_FILE, JSON.stringify(confessionsArray, null, 2));
    } catch (error) {
        console.error('❌ Error saving pending confessions:', error);
    }
}

/**
 * Load confession count from file
 */
function loadConfessionCount(): number {
    try {
        if (require('fs').existsSync(CONFESSION_COUNT_FILE)) {
            const data = require('fs').readFileSync(CONFESSION_COUNT_FILE, 'utf8');
            const { count } = JSON.parse(data);
            return count || 0;
        }
    } catch (error) {
        console.error('❌ Error loading confession count:', error);
    }
    return 0;
}

/**
 * Save confession count to file
 */
function saveConfessionCount(count: number) {
    try {
        require('fs').writeFileSync(CONFESSION_COUNT_FILE, JSON.stringify({ count }, null, 2));
    } catch (error) {
        console.error('❌ Error saving confession count:', error);
    }
}
...
// Store pending confessions with persistent storage
const pendingConfessions = new Map<string, ConfessionData>();
const CONFESSIONS_FILE = 'pending-confessions.json';
const CONFESSION_COUNT_FILE = 'confession-count.json';
